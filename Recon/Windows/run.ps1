...
#See if a loot folder exists in usb. If not create one
...
$TARGETDIR = $backupDrive + "\loot\winrecon"
if(!(Test-Path -Path $TARGETDIR )){
    New-Item -ItemType directory -Path $TARGETDIR
}
...
$backupPath = $backupDrive + "\loot\winrecon\" + $computerSystem.Name + " - " + $datetime + ".txt"
